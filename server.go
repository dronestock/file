...
type server struct {
	...
	// 地址
	Addr string `json:"addr" validate:"required,ip|hostname|hostname_port|url"`
	// 用户名
...
func (s *server) upload(plugin *plugin, uploads []*upload) (err error) {
	...
	case serverTypeWebdav:
		_uploader = newUploaderWebdav(s.Addr, s.Username, s.Password)
	case serverTypeFtp:
		_uploader, err = newUploaderFtp(s.Addr, s.Username, s.Password, s.Timeout)
	case serverTypeSsh:
